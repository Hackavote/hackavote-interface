    ...
    async function getData() {
      if (submissionInfoUrl) {
        try {
          ...
          const [description, howItsMade] = dom.querySelectorAll("div#__next div div.relative.max-w-7xl.mx-auto.px-6.w-full.py-12 div div.flex-1.space-y-8.text-black-700 div p.mt-4.mb-2")?.map(el => el.innerText)
          setProjectInfo({
            title,
            shortDescription,
            sourceCode,
